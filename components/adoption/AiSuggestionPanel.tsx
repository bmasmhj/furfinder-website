...
export function AiSuggestionPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div
      ...
      className="mx-auto mb-10 max-w-4xl scroll-mt-24 rounded-2xl border-2 border-primary/20 bg-primary/5 p-6 md:p-8"
    >
      <div className="mb-1 flex items-start justify-between gap-4">
        <div className="flex items-center gap-2">
          <Sparkles className="text-primary" size={22} />
          <h2 className="text-xl font-bold text-foreground">Not sure what pet is right for you?</h2>
        </div>
        <button
          type="button"
          onClick={onClose}
      ...
      <div className="mb-1 inline-flex rounded-lg border border-border bg-background p-1">
        {(
          [
      ...
      {result ? (
        <div className="mt-8">
          {result.breeds.length > 0 ? (
            <>
              <p className="text-sm font-semibold text-foreground">Breeds that match what you're after</p>
                ...
                {result.breeds.map((b) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "flex items-center gap-2 rounded-full border px-3 py-1.5 text-sm font-semibold capitalize transition-colors",
                        isExpanded
...
function BreedDetailCard({ breed }: { breed: BreedRecommendation }) {
  ...
  return (
    <div className="mt-4 rounded-xl border border-border bg-card p-5">
      <h3 className="text-lg font-bold capitalize text-foreground">{formatBreedName(breed.breed)}</h3>
      {breed.description ? (
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{breed.description}</p>
      ) : null}
      ...
      {breed.health_notes ? (
        <p className="mt-3 text-xs text-muted-foreground">
          <span className="font-semibold text-foreground">Health notes: </span>
          {breed.health_notes}
        </p>
      ) : null}
      ...
      {tradeoffs.length > 0 ? (
        <p className="mt-2 text-xs text-muted-foreground">
          <span className="font-semibold text-foreground">Keep in mind: </span>
          {tradeoffs.join(", ")}
        </p>
      ) : null}

      {typeof breed.explanation === "string" ? (
        <p className="mt-2 text-xs italic text-muted-foreground">{breed.explanation}</p>
      ) : null}
...
function DetailStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg bg-muted/50 px-2.5 py-2">
      <p className="font-semibold uppercase tracking-wide text-muted-foreground/70">{label}</p>
